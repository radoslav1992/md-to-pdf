...
pub fn hash_password(password: &str) -> Result<(String, String), ConvertError> {
    ...
    getrandom(&mut salt).map_err(|e| ConvertError::Internal(format!("rng: {e}")))?;
...
pub fn random_token() -> Result<String, ConvertError> {
    let mut buf = [0u8; TOKEN_LEN];
    getrandom(&mut buf).map_err(|e| ConvertError::Internal(format!("rng: {e}")))?;
    Ok(hex::encode(buf))
}
